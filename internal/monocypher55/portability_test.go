...
// TestNoHostAbsolutePaths est un garde d'auto-immunité : il scanne l'intégralité
// des sources et tests du paquet et échoue immédiatement si un chemin absolu hôte
// est détecté dans une chaîne littérale.
func TestNoHostAbsolutePaths(t *testing.T) {
	...
	err := filepath.Walk(pkgDir, func(path string, info os.FileInfo, err error) error {
		...
		// Ne pas s'auto-analyser sur la déclaration des motifs interdits
	...
	if err != nil {
		t.Fatalf("erreur lors du parcours du répertoire : %v", err)
	}
}
