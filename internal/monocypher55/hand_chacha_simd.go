...
func Crypto_chacha20_djb(cipher_text []byte, plain_text []byte, text_size uint64, key []byte, nonce []byte, ctr uint64) uint64 {
	...
	if hasAVX2() && v34 >= 4 {
		...
		if newCtr != ctr {
			v6 += simdBytes
			// v8 n'avance que si un plaintext existe : la queue partielle
			// indexe sinon le tampon `zero` (128 octets) à v8+…, hors bornes
			// dès que v8 > 64 (panique mesurée le 2026-08-15, keystream nu
			// de taille > 256 non multiple de 64).
			if plain_text != nil {
				v8 += simdBytes
			}
			v35 += simdBlocks
	...
	// Paires de blocs restantes en une passe 256-bit (2 blocs) : couvre la
	// bande 65..255 octets et les reliquats 2-3 blocs après le kernel 4x,
	// entièrement scalaires jusqu'ici (mesure 2026-08-15 : 255 o plus lent
	// que 256 o).
	for hasAVX2() && v34-v35 >= 2 {
		curCtr := uint64(v12[12]) | (uint64(v12[13]) << 32)
		if plain_text != nil {
			chacha20_xor2_simd256(cipher_text[v6:v6+128], plain_text[v8:v8+128], key, nonce, curCtr)
			v8 += 128
		} else {
			chacha20_xor2_simd256(cipher_text[v6:v6+128], nil, key, nonce, curCtr)
		}
		v6 += 128
		v35 += 2
		curCtr += 2
		v12[12] = uint32(curCtr)
		v12[13] = uint32(curCtr >> 32)
	}
