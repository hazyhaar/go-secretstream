...
func BenchmarkStratum_AEAD_LockDst_1M(b *testing.B) {
	key, nonce := keyNonce()
	pt := mkPT(1024*1024, "i%251")
	dst := make([]byte, len(pt))
	var mac [16]byte
...
func BenchmarkStratum_AEAD_UnlockDst_1M(b *testing.B) {
	key, nonce := keyNonce()
	pt := mkPT(1024*1024, "i%251")
	cipher := make([]byte, len(pt))
	plain := make([]byte, len(pt))
	var mac [16]byte
	if err := sgoi.LockDst(cipher, mac[:], key, nonce, nil, pt); err != nil {
		b.Fatal(err)
	}
	b.SetBytes(1024 * 1024)
	b.ReportAllocs()
	b.ResetTimer()
	for b.Loop() {
		if err := sgoi.UnlockDst(plain, key, nonce, mac[:], nil, cipher); err != nil {
			b.Fatal(err)
		}
	}
}
